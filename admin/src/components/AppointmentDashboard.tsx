...
export default function AppointmentDashboard() {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      // Ensure this points to your actual backend route
  ...
  const summary = useMemo(() => {
    // Single initialization with broad typing to allow mixed values (numbers and strings)
    ...
    appointments.forEach((appointment) => {
      ...
      // Increment status count if it exists in our tracker
  ...
  const statusMap = useMemo(() => {
    const map: Record<string, number> = { CONFIRMED: 0, COMPLETED: 0, CANCELLED: 0, PENDING: 0 };
    appointments.forEach((appointment) => {
      const status = appointment.status?.toUpperCase() || "CONFIRMED";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          ...
          <p className="mt-2 text-sm text-gray-500">Scheduled today and later.</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          ...
          <p className="mt-2 text-sm text-gray-500">Most booked service.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Live list with status details.</p>
          </div>

          <div className="flex items-center gap-3">
            <div className="text-sm text-gray-500">
              {lastUpdated ? `Last updated ${new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "numeric", second: "numeric" }).format(lastUpdated)}` : "Loading..."}
            </div>
            <button
              type="button"
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-gray-500">Loading appointments...</td>
                </tr>
              ) : appointments.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-gray-500">No appointments found.</td>
                </tr>
              ) : (
                appointments.map((appointment) => (
                  <tr key={appointment.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-4 text-sm text-slate-900">
                      <div className="font-medium">{appointment.service?.title || "Removed"}</div>
